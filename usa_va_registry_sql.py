...
files = [f for f in files if f.startswith('platinum_usa_va_va_registry')]
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        chunk['identifier'] = chunk['identifier'].apply(lambda x: x+'_VA')
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        chunk.fillna("", inplace=True)
        chunk = chunk[chunk['activity_name'] != ""]
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'registry_number': 'identifier', 'person':'person_name'}, inplace=True)
        chunk.fillna("", inplace=True)
