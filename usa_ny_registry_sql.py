...
files = [f for f in files if f.startswith('platinum_usa_ny_ny_registry')]
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'registry_number': 'identifier', 'Category':'activity_name', 'issued_date':'activity_date'}, inplace=True)
        chunk.fillna("", inplace=True)
