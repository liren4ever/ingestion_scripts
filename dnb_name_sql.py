...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(pd.read_csv(csv_path, chunksize=chunk_size, dtype='str', usecols=['identifier', 'name', 'first_time_check']), desc="Processing chunks"):
        chunk = chunk.copy()
        chunk = chunk[~chunk['name'].isna()]
        chunk['name_type'] = 'main'
        chunk['last_time_check'] = '2023-12-20'
        chunk.rename(columns={'name': 'business_name'}, inplace=True)
