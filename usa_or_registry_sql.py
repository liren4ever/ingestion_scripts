...
files = [f for f in files if f.startswith('platinum_usa_or_or_registry')]
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        chunk.rename(columns={'address_en': 'address', 'region_code': 'state', 'postal_code': 'postal', 'country_code': 'country', "lon":"latitude", "lat":"longitude"}, inplace=True)
...
#         chunk = chunk.copy()
#         chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
#         chunk.rename(columns={'alt_business_names':'business_name'}, inplace=True)
#         chunk.fillna("", inplace=True)
#         chunk = chunk[chunk['business_name'] != ""]
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'Category':'activity_name', 'creation_year':'activity_date'}, inplace=True)
        chunk.fillna("", inplace=True)
