...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(pd.read_csv(csv_path, chunksize=chunk_size, dtype='str', usecols=['identifier', 'phone', 'first_time_check']), desc="Processing chunks"):
        chunk = chunk.copy()
        chunk = chunk[chunk['phone'].str.contains(r'\d', na=False)]
        chunk['phone'] = chunk['phone'].str.replace(r'\D', '', regex=True)
        chunk['last_time_check'] = formatted_date
