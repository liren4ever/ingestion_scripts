...
files = [f for f in files if f.startswith('platinum_usa_ct_ct_registry')]
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        desc="Processing location",
    ):
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'registry_number': 'identifier', 'address_en':'address', 'region_code': 'state', 'postal_code': 'postal', 'country_code': 'country', "lon":"latitude", "lat":"longitude"}, inplace=True)
        ...
        chunk['identifier'] = chunk['identifier'].apply(lambda x: x+'_CT')
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        desc="Processing alt location",
    ):
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'registry_number': 'identifier', 'alt_address_en': 'address', 'alt_city': 'city', 'alt_region_code': 'state', 'alt_postal_code': 'postal', 'alt_country_code': 'country', "alt_address_type":"location_type"}, inplace=True)
        chunk.fillna("", inplace=True)
        chunk = chunk[chunk['address'] != ""]
        chunk = chunk[chunk['country'] != ""]
        chunk['city'] = chunk['city'].apply(lambda x: str(x).lower())
        chunk['location_type'] = chunk['location_type'].apply(lambda x: str(x).split(' ')[0].lower())
        chunk["state"] = chunk.apply(
            lambda row: row["state"].replace(row["country"][0:2]+'-', ""), axis=1
        )
        chunk["postal"] = chunk["postal"].apply(lambda x: str(x).replace(" ", ""))
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        desc="Processing business name",
    ):
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'registry_number': 'identifier'}, inplace=True)
        chunk.fillna("", inplace=True)
...
#             ],
#         ),
#         desc="Processing alt business name",
#     ):
#         chunk = chunk.copy()
#         chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
#         chunk.rename(columns={'registry_number': 'identifier', 'alt_business_names':'business_name'}, inplace=True)
#         chunk.fillna("", inplace=True)
#         chunk = chunk[chunk['business_name'] != ""]
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        desc="Processing identifier",
    ):
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'registry_number': 'identifier', 'registry_status':'status'}, inplace=True)
        chunk.fillna("", inplace=True)
...
with tqdm(total=total_chunks, desc="Processing chunks") as pbar:
    for chunk in tqdm(
        ...
        desc="Processing activity",
    ):
        chunk = chunk.copy()
        chunk = chunk.apply(lambda x: x.str.strip() if x.dtype == 'object' else x)
        chunk.rename(columns={'registry_number': 'identifier', 'Category':'activity_name', 'creation_year':'activity_date'}, inplace=True)
        chunk.fillna("", inplace=True)
